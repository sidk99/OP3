...
class PhysicsNetworkNoAttention(nn.Module):
    def __init__(
            self,
            K,
            representation_size,
...
# #######All at once ablation########
class PhysicsNetworkAllAtOnce(nn.Module):
    def __init__(
            self,
            K,
            representation_size,
        ...
        self.all_at_once_net((hidden_size, hidden_size, hidden_size, hidden_size,),
                             self.rep_size, self.rep_size*2 + self.action_size, hidden_activation=nn.ELU())
        ...
        if K != 1:
            ...
            for i in range(K):
                for j in range(K):
                    if i == j:
                        continue
                    pairs.append(torch.cat([lambda1[:, i], lambda1[:, j]], -1))  # Create array of all pairs

            all_pairs = torch.stack(pairs, 1).view(bs * K, K - 1, -1)  #(B*K,K-1,2R)

        if self.action_size == 0:
            input = all_pairs
        elif self.action_size == 4:
            actions = actions[:, torch.LongTensor([0, 1, 3, 4])]  #(B*K,A)
            input = torch.cat([all_pairs, actions], -1)  # (B*K,K-1,2R+A)
        else:
            input = torch.cat([all_pairs, actions], -1) #(B*K,K-1,2R+A)

        lambda1 = self.all_at_once_net(input)  #(B*K,K-1,R)
        lambda1 = lambda1.sum(1) #(B*K,R)
        return lambda1, None
#
#
#
