...
def get_object_subimage_recon(frames, actions, model, model_type, T):
    ...
    if model_type == 'static':
        ...
        for i in range(T):
            ...
            # object_recons = torch.where(masks < 0.05, ptu.ones_like(object_recons), object_recons)
            object_recons = object_recons[:, -1]  # (bs, K, 3, D, D)
        ...
        mse = get_image_mse(frames[:, :T], all_object_recons[:, :, 0]) #(bs, T)
        return all_object_recons, mse
    elif model_type == 'rprp_next_step':
        # T is the total number of frames, so we do T-1 physics steps
        num_refine_per_phys = 4
        schedule = np.zeros(seed_steps + (T - 1) * num_refine_per_phys)  # len(schedule) = T2
        schedule[seed_steps::num_refine_per_phys] = 1  # [0,0,0,0,1,0,1,0,1,0] if num_refine_per_phys=2 for example
...
def create_image(models_and_type, frames, actions, image_prefix, T):
    ...
    for model, model_type in models_and_type:
        object_recons, _ = get_object_subimage_recon(frames, actions, model, model_type, T) #(bs, T, K+1, 3, D, D)
...
def get_mse(models_and_type, frames, actions, T):
    ...
    all_mse = []
    for model, model_type in models_and_type:
        all_object_recons, mse = get_object_subimage_recon(frames, actions, model, model_type, T) #mse is (bs, T)
        all_mse.append(mse)
...
def get_mse_from_dataset(variant):
    ...
    for i in range(len(models_and_type)):
        if models_and_type[i][1] == 'next_step':
            plt.errorbar(range(1, variant['T']), mean_vals[i][1:], std_vals[i][1:], label='{}'.format(models_and_type[i][1]), capsize=5)
        else:
            plt.errorbar(range(variant['T']), mean_vals[i], std_vals[i], label='{}'.format(models_and_type[i][1]), capsize=5)
...
def analyze_mse(variant):
    if variant['dataset'] == 'cloth':
        ...
        file_path = '/nfs/kun1/users/rishiv/Research/op3_exps/07-04-images-solid/07-04-images-solid_2019_07_04_03_37_11_0000--s-18011/computed_mse.npy'
    else:
...
#Poke models:
# models = [dict(model=iodine.imsize64_large_iodine_architecture_multistep_physics, K=4,
#                        model_file="/nfs/kun1/users/rishiv/Research/op3_exps/06-28-poke-rprp/06-28-poke-rprp_2019_06_28_23_13_26_0000--s-13560/_params.pkl",
#                        model_type="rprp"),
#                   dict(model=iodine.imsize64_large_iodine_architecture_multistep_physics, K=4,
...
#Solid models:
# models = [dict(model=iodine.imsize64_large_iodine_architecture_multistep_physics, K=4,
#                        model_file="/nfs/kun1/users/rishiv/Research/op3_exps/07-01-solid-rprp/07-01-solid-rprp_2019_07_01_08_37_37_0000--s-63717/_params.pkl",
#                        model_type="rprp"),
#                   dict(model=iodine.imsize64_large_iodine_architecture_multistep_physics, K=4,
...
#Example usage: CUDA_VISIBLE_DEVICES=7 python visualize_datasets.py -da solid
if __name__ == "__main__":
    ...
    run_experiment(
        create_images_from_dataset, #get_mse_from_dataset, create_images_from_dataset, analyze_mse
